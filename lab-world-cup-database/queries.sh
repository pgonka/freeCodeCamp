...
echo $($PSQL "SELECT (AVG(winner_goals) + AVG(opponent_goals))::numeric(20,16) FROM games;")
...
echo "$($PSQL "SELECT year, name FROM teams INNER JOIN games ON teams.team_id = games.winner_id WHERE round='Final' ORDER BY year, name;")"
